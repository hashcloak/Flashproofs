 ...
 *
 */
public class RangeZKPK4 extends PedersenZKP {
	...
	public RangeZKPK4(BigInteger y, int nbits, int L) {
		...
		List<List<Integer>> combinations = Arrays.asList(Arrays.asList(2, 3), Arrays.asList(1, 3), Arrays.asList(0, 2));
		...
		for (int i = 0; i < combinations.size(); i++) {
			...
			for (int j = 0; j < this.L; j++) {
				...
				ts.add(t.compareTo(this.n) < 0 && t.compareTo(BigInteger.ZERO) >= 0 ? t : t.mod(this.n));
			}
		...
		for (int i = 0; i < this.L; i++) {
			BigInteger t = ls.get(3).get(i);
			t = t.add(bs.get(i).get(1).multiply(bsr.get(i).get(2)));
			t = t.add(bs.get(i).get(2).multiply(bsr.get(i).get(1)));
			t = (t.compareTo(this.n) < 0 && t.compareTo(BigInteger.ZERO) >= 0) ? t : t.mod(this.n);
			ls.get(3).set(i, t);
			
			t = ls.get(2).get(i);
			t = t.add(bs.get(i).get(0).multiply(bsr.get(i).get(3)));
			t = t.add(bs.get(i).get(3).multiply(bsr.get(i).get(0)));
			t = (t.compareTo(this.n) < 0 && t.compareTo(BigInteger.ZERO) >= 0) ? t : t.mod(this.n);
			ls.get(2).set(i, t);
		}
		...
		doublechallenges.add(challenge9);
		doublechallenges.add(challenge6);
