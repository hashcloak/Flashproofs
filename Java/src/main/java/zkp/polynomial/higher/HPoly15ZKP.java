 ...
 *
 */
public class HPoly15ZKP extends PedersenZKP {
	...
	public HPoly15ZKP(BigInteger x, BigInteger y, int numOfGs, int numOfGl) {
		...
		BigInteger challenge4 = challenge2.multiply(challenge2).mod(this.n);
		...
		this.fs.add(w3.multiply(challenge).add(w2).add(ls.get(1)).mod(this.n));
		...
		this.q = ry.multiply(challenge4).add(rls.get(1).multiply(challenge2)).add(rls.get(0)).mod(this.n);
	...
	public boolean verify() {
		...
		Commitment ret4_2 = this.cy.mul(challenge4).add(this.cls.get(1).mul(challenge2)).add(this.cls.get(0));
