 ...
 *
 */
public class HPoly127ZKP extends PedersenZKP {
	...
	private final int exponent = 127;

	public HPoly127ZKP(BigInteger x, BigInteger y, int numOfGs, int numOfGl) {
		...
		this.cv0 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(m2_1, m2_2, m2_3, m2_4, m2_5, m2_6), this.n))
				.add(this.commiter.mulH(rv0));
		BigInteger rv1 = this.commiter.rand();
		this.cv1 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(xm1, xm2, xm3, xm4, xm5, xm6), this.n))
				.add(this.commiter.mulH(rv1));

		BigInteger w0 = ms.stream().reduce(BigInteger.ONE, (m_1, m_2) -> m_1.multiply(m_2)).mod(this.n);

		Set<Integer> set = ImmutableSet.of(0, 1, 2, 3, 4, 5, 6);
		List<Integer> weights = Arrays.asList(1, 2, 4, 8, 16, 32, 64);

		List<Set<Integer>> combinations = Sets.combinations(set, 6).stream().collect(Collectors.toList());
		...
		this.cws.add(this.gl.mulBAndSum(VectorB.from(Arrays.asList(w0.add(ls.get(0)), w3.add(ls.get(1))), this.n))
				.add(this.commiter.mulH(rw0)));

		BigInteger rw1 = this.commiter.rand();
		this.cws.add(this.gl.mulBAndSum(VectorB.from(Arrays.asList(w1, w4), this.n)).add(this.commiter.mulH(rw1)));

		BigInteger rw2 = this.commiter.rand();
		this.cws.add(this.gl.mulBAndSum(VectorB.from(Arrays.asList(w2, w5), this.n)).add(this.commiter.mulH(rw2)));

		BigInteger rw3 = this.commiter.rand();
		this.cws.add(this.gl.mulBAndSum(VectorB.from(Arrays.asList(BigInteger.ZERO, w6), this.n))
				.add(this.commiter.mulH(rw3)));
		...
		BigInteger challenge7 = challenge3.multiply(challenge3).multiply(challenge).mod(this.n);
		...
		this.fs.add(
				w6.multiply(challenge3).add(w5.multiply(challenge2).add(w4.multiply(challenge).add(w3).add(ls.get(1)))).mod(this.n));
		...
		this.q = ry.multiply(challenge7).add(rls.get(1).multiply(challenge3)).add(rls.get(0)).mod(this.n);
	...
	public boolean verify() {
		...
		Commitment ret2_1 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(
				...
				z32.pow(2).subtract(z64.multiply(challenge)).mod(this.n)), this.n)).add(this.commiter.mulH(this.u));
		...
		BigInteger zprod = this.zs.stream().reduce(BigInteger.ONE, (z_1, z_2) -> z_1.multiply(z_2))
				.subtract(this.fs.get(1).multiply(challenge3).add(this.fs.get(0))).mod(this.n);
		Commitment ret4_1 = this.commiter.commitTo(zprod, this.q);
		Commitment ret4_2 = this.cy.mul(challenge7).add(this.cls.get(1).mul(challenge3)).add(this.cls.get(0));
