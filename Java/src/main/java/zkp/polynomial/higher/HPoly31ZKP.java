 ...
 *
 */
public class HPoly31ZKP extends PedersenZKP {
	...
	private final int exponent = 31;

	public HPoly31ZKP(BigInteger x, BigInteger y, int numOfGs, int numOfGl) {
		...
		this.cv0 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(m2_1, m2_2, m2_3, m2_4), this.n))
				.add(this.commiter.mulH(rv0));
		BigInteger rv1 = this.commiter.rand();
		this.cv1 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(xm1, xm2, xm3, xm4), this.n))
				.add(this.commiter.mulH(rv1));

		BigInteger w0 = ms.stream().reduce(BigInteger.ONE, (m_1, m_2) -> m_1.multiply(m_2)).mod(this.n);

		Set<Integer> set = ImmutableSet.of(0, 1, 2, 3, 4);
		List<Integer> weights = Arrays.asList(1, 2, 4, 8, 16);

		List<Set<Integer>> combinations = Sets.combinations(set, 4).stream().collect(Collectors.toList());
		...
		this.cws.add(this.gl.mulBAndSum(VectorB.from(Arrays.asList(w0.add(ls.get(0)), w2.add(ls.get(1))), this.n))
				.add(this.commiter.mulH(rw0)));

		BigInteger rw1 = this.commiter.rand();
		this.cws.add(this.gl.mulBAndSum(VectorB.from(Arrays.asList(w1, w3), this.n)).add(this.commiter.mulH(rw1)));

		BigInteger rw2 = this.commiter.rand();
		this.cws.add(this.gl.mulBAndSum(VectorB.from(Arrays.asList(BigInteger.ZERO, w4), this.n))
				.add(this.commiter.mulH(rw2)));
		...
		BigInteger challenge5 = challenge2.multiply(challenge2).multiply(challenge).mod(this.n);
		...
		this.fs.add(w1.multiply(challenge).add(w0).add(ls.get(0)).mod(this.n));
		this.fs.add(w4.multiply(challenge2).add(w3.multiply(challenge).add(w2)).add(ls.get(1)).mod(this.n));
		...
		this.q = ry.multiply(challenge5).add(rls.get(1).multiply(challenge2)).add(rls.get(0)).mod(this.n);
	...
	public boolean verify() {
		...
		Commitment ret2_1 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(
				...
				this.n)).add(this.commiter.mulH(this.u));
		...
		BigInteger zprod = this.zs.stream().reduce(BigInteger.ONE, (z_1, z_2) -> z_1.multiply(z_2))
				.subtract(this.fs.get(1).multiply(challenge2).add(this.fs.get(0))).mod(this.n);
		Commitment ret4_1 = this.commiter.commitTo(zprod, this.q);
		Commitment ret4_2 = this.cy.mul(challenge5).add(this.cls.get(1).mul(challenge2)).add(this.cls.get(0));
