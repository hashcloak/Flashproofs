 ...
 *
 */
public class HPoly63ZKP extends PedersenZKP {
	...
	private final int exponent = 63;

	public HPoly63ZKP(BigInteger x, BigInteger y, int numOfGs, int numOfGl) {
		...
		this.cv0 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(m2_1, m2_2, m2_3, m2_4, m2_5), this.n))
				.add(this.commiter.mulH(rv0));
		BigInteger rv1 = this.commiter.rand();
		this.cv1 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(xm1, xm2, xm3, xm4, xm5), this.n))
				.add(this.commiter.mulH(rv1));

		BigInteger w0 = ms.stream().reduce(BigInteger.ONE, (m_1, m_2) -> m_1.multiply(m_2)).mod(this.n);

		Set<Integer> set = ImmutableSet.of(0, 1, 2, 3, 4, 5);
		List<Integer> weights = Arrays.asList(1, 2, 4, 8, 16, 32);

		List<Set<Integer>> combinations = Sets.combinations(set, 5).stream().collect(Collectors.toList());
		...
		this.q = ry.multiply(challenge6).add(rls.get(1).multiply(challenge3)).add(rls.get(0)).mod(this.n);
	...
	public boolean verify() {
		...
		Commitment ret2_1 = this.gs.mulBAndSum(VectorB.from(Arrays.asList(
				...
				z16.pow(2).subtract(z32.multiply(challenge)).mod(this.n)), this.n)).add(this.commiter.mulH(this.u));
		...
		Commitment ret4_2 = this.cy.mul(challenge6).add(this.cls.get(1).mul(challenge3)).add(this.cls.get(0));
