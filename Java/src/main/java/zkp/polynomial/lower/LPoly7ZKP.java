 ...
 *
 */
public class LPoly7ZKP extends PedersenZKP {
	...
	public LPoly7ZKP(BigInteger x, BigInteger y, int numberOfGs) {
		...
		Set<Integer> set = ImmutableSet.of(0, 1, 2);
		List<Integer> weights = Arrays.asList(1, 2, 4);
		...
		this.s = ry.multiply(challenge3).add(rw2.multiply(challenge2)).add(rw1.multiply(challenge)).add(rw0).mod(this.n);
	...
	public boolean verify() {
		...
		Commitment ret3_2 = this.cy.mul(challenge3).add(this.cws.get(2).mul(challenge2)).add(this.cws.get(1).mul(challenge))
				.add(this.cws.get(0));
